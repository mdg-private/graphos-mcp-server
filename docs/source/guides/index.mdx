...
You can specify a custom scalars configuration JSON file to map a custom scalar to a [JSON schema type](https://json-schema.org/understanding-json-schema/reference/type). The JSON file contains an object with the keys being the custom scalar name and the values the JSON schema type:
```json
{
